...
def fft_2d(image, width, height):
    ...
    return fft_image, abs_fft, abs_fft_shifted, freq_x, freq_y, freq_x_grid_shifted, freq_y_grid_shifted
...
def contour_plot(theta, circle_center, R):
    ...
    plt.contourf(X, Y, np.rad2deg(theta)[y_min:y_max, x_min:x_max], levels=20)
    plt.colorbar()
...
def plot_theta_central_row(to_keep, width, edge_circ_cols, angle_name, thetas, theta_calibrated, radius_of_circle, circle_center):
    ...
    plt.plot([circle_center[1]+radius_of_circle, circle_center[1]+radius_of_circle], [theta_min_plot, theta_max_plot], linestyle='-', color='black', label='Edge of circle (for 0.25/0.75 D)')
...
def plot_theta_cols(height, theta_calibrated, angle_name, thetas, radius_of_circle, circle_center):
    ...
    print('cols to plot: ', cols_to_plot)
...
if __name__ == '__main__':
    ...
    plt.subplot(1,2,1)
    plt.imshow(np.log(abs_fft+1), origin='lower')
    ...
    #     plt.title('Original Image with circle boundary (uncropped!)')
    ...
    # plot_all_thetas(theta_out_1_pp=theta_out_1_pp, theta_out_1_pm=theta_out_1_pm, theta_out_1_arith=theta_out_arith, theta_out_2=theta_out_2, edge_rows=edge_circ_rows, edge_cols=edge_circ_cols)
    # plt.show()
